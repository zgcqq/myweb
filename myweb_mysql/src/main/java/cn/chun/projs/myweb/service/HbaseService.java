...
import cn.chun.projs.myweb.config.HbaseConfig;
...
public class HbaseService {
    ...
    public JSONArray scanAllTable(String tableName) throws IOException {
        JSONArray result = new JSONArray();
        Table table = connection.getTable(TableName.valueOf(tableName));
        Scan scan = new Scan();
        ResultScanner scanner = table.getScanner(scan);
        try {
            for (Result scanOne : scanner) {
                System.out.println(Bytes.toString(scanOne.getRow()));
                for (Cell cell : scanOne.rawCells()) {
